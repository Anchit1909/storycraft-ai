...
export class BackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: BackendStackProps) {
    ...
    const createStoryFunc: NodejsFunction = new NodejsFunction(
      this,
    ...
    const createImagesForStoryFunc: NodejsFunction = new NodejsFunction(
      this,
      "generate-images-for-story",
      {
        ...
        entry: path.join(
          __dirname,
          "../src",
          "generate-images-for-story/index.ts"
        ),
        environment: {
          openAIARN: openAIAPIKEY.secretArn,
          STORIES_TABLE: generatedStories.tableName,
    ...
    const createAudioForStoryFunc: NodejsFunction = new NodejsFunction(
      this,
      "generate-audio-for-story",
      {
        ...
        entry: path.join(
          __dirname,
          "../src",
          "generate-audio-for-story/index.ts"
        ),
        environment: {
          STORIES_TABLE: generatedStories.tableName,
          BUCKET_NAME: audioBucket.bucketName,
    ...
    storyGeneratedRule.addTarget(
      new SnsTopic(emailTopic, {
        message: RuleTargetInput.fromText(
          `New Story created: ${
            props.frontEndURL
          }/story?id=${EventField.fromPath("$.detail.id")}`
        ),
      })
    );
